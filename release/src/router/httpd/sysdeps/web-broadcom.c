...
#if defined(RTCONFIG_BCMWL6) && !defined(RTCONFIG_HND_ROUTER_BE_4916)
...
#if defined(RTCONFIG_HND_ROUTER_AX) && (defined(RTCONFIG_WIFI6E) || defined(RTCONFIG_WIFI7))
...
#endif //RTCONFIG_HND_ROUTER_AX && (defined(RTCONFIG_WIFI6E) || defined(RTCONFIG_WIFI7))
	} else {
		/* invalid band */
		return TRUE;
	}
 ...
 *
 * @return a string containing 10 chars (excluding \n)
 */
static char *
wl_eht_cap_mcs_map_to_str(uint16 mcs_bitmap)
{
	static char buf[11];
	uint num = 0;

	switch (mcs_bitmap & WL_EHT_CAP_MCS_0_13_MAP) {
	case WL_EHT_CAP_MCS_0_13_MAP:
		strncpy(buf, "0-13", sizeof(buf));
		break;
	case WL_EHT_CAP_MCS_0_11_MAP:
		strncpy(buf, "0-11", sizeof(buf));
		break;
	case WL_EHT_CAP_MCS_0_9_MAP:
		strncpy(buf, "0-9", sizeof(buf));
		break;
	case WL_EHT_CAP_MCS_0_7_MAP:
		strncpy(buf, "0-7", sizeof(buf));
		break;
	case 0:
		return "---       ";
	default:
	...
	if (mcs_bitmap & BCM_BIT(14)) { /* DCM + DUP */
		strncat(buf, ",14", sizeof(buf) - strlen(buf));
	}

	if (mcs_bitmap & BCM_BIT(15)) { /* DCM */
		strncat(buf, ",15", sizeof(buf) - strlen(buf));
	}

	/* append spaces */
	for (num = strlen(buf); num < sizeof(buf); num++) {
		buf[num] = ' ';
	}
	buf[sizeof(buf)-1] = '\0';

	return buf;
} /* wl_eht_cap_mcs_map_to_str */
 ...
 */
static int
wl_print_eht_rateset(webs_t wp, uint16 (*p)[WL_RXTX_SZ][WL_EHT_BW_SZ][EHT_MCS_MAP_NSS_MAX])
{
	uint nss;
	enum wl_eht_bw_e bw_idx;
	uint16 tx_mcsbitmap, rx_mcsbitmap;
	static const uint16 zero[WL_RXTX_SZ][WL_EHT_BW_SZ][EHT_MCS_MAP_NSS_MAX];
	char *rx_mcs_str, *tx_mcs_str;
	char *bw_str_array[WL_EHT_BW_SZ] = {"20/40/80 MHz", "160 MHz", "320 MHz"};
	int retval = 0;
	...
	for (bw_idx = WL_EHT_BW204080; bw_idx <= WL_EHT_BW320; bw_idx++) {
		bool bw_printed = FALSE;
		for (nss = 1; nss <= 8; nss++) {
			tx_mcsbitmap = dtoh16((*p)[WL_TX][bw_idx][nss - 1]);
			rx_mcsbitmap = dtoh16((*p)[WL_RX][bw_idx][nss - 1]);
			if ((rx_mcsbitmap & WL_EHT_CAP_MCS_0_13_MAP) ||
				(tx_mcsbitmap & WL_EHT_CAP_MCS_0_13_MAP)) {
				tx_mcs_str = wl_eht_cap_mcs_map_to_str(tx_mcsbitmap);
				rx_mcs_str = wl_eht_cap_mcs_map_to_str(rx_mcsbitmap);
				if (!bw_printed) {
					retval += websWrite(wp, "\t    %s:\n", bw_str_array[bw_idx]);
					bw_printed = TRUE;
				}
				retval += websWrite(wp, "\t\tNSS%d Tx: %s  Rx: %s\n", nss, tx_mcs_str, rx_mcs_str);
			}
		}
	}

	return retval;
} /* wl_print_eht_rateset */
#endif
#ifdef RTCONFIG_HND_ROUTER_AX
static int
...
wl_print_hemcsnss(int eid, webs_t wp, int argc, char_t **argv, uint16 *mcsset)
{
	...
	for (bw_idx = 0; bw_idx < 3; bw_idx++) {
		...
		he_txmcsmap = dtoh16(mcsset[bw_idx * 2]);
		/* get he bw80, bw160, bw80p80 rx mcs from mcsset[1], mcsset[3], and mcsset[5] */
		he_rxmcsmap = dtoh16(mcsset[(bw_idx * 2) + 1]);

		for (nss = 1; nss <= HE_CAP_MCS_MAP_NSS_MAX; nss++) {
#ifdef RTCONFIG_WIFI7
			tx_mcs = HE_MCS_MAP_GET_MCS(nss, he_txmcsmap);
			rx_mcs = HE_MCS_MAP_GET_MCS(nss, he_rxmcsmap);
#else
...
//	retval += websWrite(wp, "Mode: %s\t", capmode2str(dtoh16(bi->capability)));
	...
	retval += websWrite(wp, "Beacon Interval: %u msecs\t", dtoh16(bi->beacon_period));
	if (flags & WL_BSS_FLAGS_FROM_BEACON)
		retval += websWrite(wp, "DTIM Period: %u", bi->dtim_period);
	retval += websWrite(wp, "\n");
...
#ifdef RTCONFIG_WIFI7
	if (version != LEGACY_WL_BSS_INFO_VERSION && (bi->mlo_cap)) {
		retval += websWrite(wp, "MLO: MLD Address: %s sim_links: %d\n",
			wl_ether_etoa(&bi->mld_addr), bi->mlo_sim_links);
	}
#endif
...
#ifdef RTCONFIG_WIFI7
		if(bi->eht_cap) {
			retval += websWrite(wp, "EHT Capable:\n");
		} else
#endif
#ifdef RTCONFIG_HND_ROUTER_AX
		if (bi->he_cap) {
...
#ifdef RTCONFIG_WIFI7
			retval += websWrite(wp, "\tBSS Color: 0x%x\t", bi->bsscolor_info & WLC_BSS_COLOR_MASK);

			if (bi->bsscolor_info & WLC_PARTIAL_BSS_COLOR_MASK) {
				retval += websWrite(wp, "\tPartial BSS Color: true\n");
			} else {
				retval += websWrite(wp, "\tPartial BSS Color: false\n");
			}

			if (bi->bsscolor_info & WLC_BSS_COLOR_DISABLE_MASK) {
				retval += websWrite(wp, "\tBSS Color Disabled: true\n");
			} else {
				retval += websWrite(wp, "\tBSS Color Disabled: false\n");
			}
#endif
		}
#endif
#ifdef RTCONFIG_WIFI7
#if WL_EHT_IOCTL_VER >= 1 /* then struct wl_bss_info has EHT members */
                if (bi->eht_cap) {
                        uint16 (*p)[WL_RXTX_SZ][WL_EHT_BW_SZ][EHT_MCS_MAP_NSS_MAX];

                        if (bi->eht_neg[WL_TX][WL_EHT_BW204080][0] != WLC_EHT_MCS_MAP_MCS_NONE) {
                                retval += websWrite(wp, "\tNegotiated EHT MCS:\n");
                                p = &bi->eht_neg;
                        } else {
                                retval += websWrite(wp, "\tSupported EHT MCS:\n");
                                p = &bi->eht_sup;
                        }

			retval += wl_print_eht_rateset(wp, p);
                }
...
#if (WL_STA_VER >= 7)
#if defined(RTCONFIG_HND_ROUTER_BE_4916)
	sta_info_t *sta;	/* typedef sta_info_v8_t in wlc_types.h */
#else
	sta_info_v5_t *sta;
...
#define PHY_TYPE_BE     6
#define PHY_TYPE_MAX	7
...
#if defined(RTCONFIG_WIFI7)
#if WL_EHT_IOCTL_VER >= 2
	if (sta->flags & WL_STA_EHT_CAP)
		return PHY_TYPE_BE;
#endif /* WL_EHT_IOCTL_VER */
#endif /* RTCONFIG_WIFI7 */
