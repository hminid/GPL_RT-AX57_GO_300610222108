...
#ifdef CONFIG_AIROHA_EN8801SC_PHY
#include <asm/gpio.h>
#include "en8801sc.h"
#endif
...
#ifdef CONFIG_AIROHA_EN8801SC_PHY
	if (phydev->phy_id == (EN8801S_PHY_ID-0x10)) {
		genphy_parse_link(phydev);
		en8801s_update_status(phydev);
	}
#endif
	return 0;
}
...
int __weak get_phy_id(struct mii_dev *bus, int addr, int devad, u32 *phy_id)
{
	...
	if (addr != 0) {
		ofnode node;
		node = ofnode_by_compatible(ofnode_null(), "ethernet-phy-id03a2.9471");
	        if (ofnode_valid(node) && ofnode_read_u32_default(node, "reg", 0) == addr) {
#if 0 /* save time */
			struct gpio_desc gpio = {};
			if (gpio_request_by_name_nodev(node, "reset-gpios", 0, &gpio, GPIOD_IS_OUT)==0) {
				// printf("\nVVVVVV, reset-gpios:%d, deassert \n", gpio.offset);
				dm_gpio_set_value(&gpio, 0);
				mdelay(1000);
				// printf("VVVVVV, reset-gpios:%d, assert \n", gpio.offset);
				dm_gpio_set_value(&gpio, 1);
				mdelay(1000);
				// printf("VVVVVV, reset-gpios:%d, finish \n", gpio.offset);
			}
#endif
			en8801s_init(bus, addr);
		}
	}
#endif
