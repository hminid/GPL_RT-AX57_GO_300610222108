...
#if defined(CONFIG_ASUS_PRODUCT)
	if (images.fit_rfs_offset)
		sprintf(commandline_buf, "root_rfs=0x%lx ", images.fit_rfs_offset);
	else
		commandline_buf[0] = '\0';

	p = commandline_buf;
	strcat(p, "rootfstype=squashfs ubi.mtd=UBI_DEV ");

        str = (char *)fdt_getprop(fdt, nodeoffset, "bootargs", &len);
	if (str) {
		strcat(p, str);
		strcat(p, " ");
	}
...
void fdt_fixup_ethernet(void *fdt)
{
	...
	for (prop = 0; ; prop++) {
...
#endif
			if (i != -1) {
#if defined(CONFIG_ASUS_PRODUCT) // 2G/LAN/WAN use the same MAC
				strcpy(mac, "ethaddr");
#else
				if (i == 0)
					strcpy(mac, "ethaddr");
		...
		else if (!strncmp(name, "switchport", 10)) {
			int off, toff, ret;
			char *id;

			id = env_get("HwID");
			if (id==NULL || strcmp(id, "A")!=0)
				continue;

			toff = fdt_path_offset(fdt, path);
			for (off = fdt_first_subnode(fdt, toff);
			     off >= 0;
			     off = fdt_next_subnode(fdt, off)) {
				if (strcmp(fdt_get_name(fdt, off, NULL), "port@5") == 0) {
					ret = fdt_del_node(fdt, off);
					if (ret < 0)
						printf("Can't remove port@5: %s\n", fdt_strerror(ret));
				}
			}
		}
#endif
