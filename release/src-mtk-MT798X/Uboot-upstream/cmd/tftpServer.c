...
#endif
#ifdef RESCUE_SWITCH2_RESET_DEFAULT
int gl_switch_val;
int gl_switch_val_xcount;
#endif
...
void switch_r2default_detect()
{
	int val = DETECT_SWITCH();
	if ( val != gl_switch_val ) {
		gl_switch_val_xcount++;
		gl_switch_val = val;
	}
	if ( gl_switch_val_xcount >= 2 ) {
		int i = 0;
		printf("\n\n!!!! Got switching to reset default !!!!\n\n");
#if defined(CONFIG_PWM_MTK_MM)
		bbtype(0);
#endif
		reset_to_default();
...
#endif
#ifdef RESCUE_SWITCH2_RESET_DEFAULT
		gl_switch_val = DETECT_SWITCH();
		gl_switch_val_xcount = 0;
		//printf("[%s:%d] switch_val:%d\n", __func__, __LINE__, gl_switch_val);
#endif
