				...
				struct image_tool_params *params)
{
	...
	if (params->vargv) {
		int len = strlen(params->imagename);
		if (len > MAX_STRING) {
			fprintf(stderr, "imagename[%s] len[%d] exceeds[%d]\n", params->imagename, len, MAX_STRING);
			exit(-1);
		}
		memcpy((char *)params->tail_pre.productid, params->imagename, len);
